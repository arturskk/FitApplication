...
public final class FoodService {
	...
	public final FoodEatenDTO findFoodEatenById(@Nonnull final Integer foodEatenId) {
		final FoodEaten foodEaten = foodEatenRepository.findById(foodEatenId).orElse(new FoodEaten());
	...
	public final FoodEatenDTO addFoodEaten(
		...
		if (!duplicate) {
			final FitnessUser user = userRepository.findById(userId).orElse(new FitnessUser());
			final Food food = foodRepository.findById(foodId).orElse(new Food());
		...
		if (foodDTO.getOwnerId() == null || foodDTO.getOwnerId().equals(userDTO.getId())) {
			...
			if (noConflictsFound) {
				...
				if (foodDTO.getOwnerId() == null) {
					...
					food = foodRepository.findById(foodDTO.getId()).orElse(new Food());
